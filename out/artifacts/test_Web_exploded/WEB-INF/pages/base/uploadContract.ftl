					...
					<tr>
						<td colspan="2" style="padding:10px;">
							...
							<#assign imageWidthFTL="400">
					  		<#include "/widgets/imageUploaderContract.ftl"/>
						</td>
					</tr>
...
$(function(){
	$("#add").click(function(){
	setUploaderVal();
	var contractPic= $("#image").val();
	var contractId=$("#contractId").val();
	
	layer.confirm('确认上传?',function(){
	
	$.ajax({
			url : "${contextPath}/contract/uploadContract",
			type : "post",
			async: false,
			dataType:"json",
			data:{"contractPic":contractPic,"contractId":contractId},
