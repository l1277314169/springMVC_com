	...
	<div id="table_info" style="margin-top:300px;"></div>
    <script type="text/javascript">
		   ...
		   function drawAlertInfo(val,index){
		   		$("#table_info").append(index+"："+val+"；&nbsp;&nbsp;");
		   }
		   		...
		   		if(null!=data.buttomData && ""!=data.buttomData){
			   		...
			   		jQuery.each(xAxis0, function(index, val) {
			   			var v = $("#${parts.partId}_hid").val();
			   		  	option_${parts.partId}.xAxis[0].data.push((++index)+v);
			   		  	drawAlertInfo(val,index);
			   		});
		   		}
