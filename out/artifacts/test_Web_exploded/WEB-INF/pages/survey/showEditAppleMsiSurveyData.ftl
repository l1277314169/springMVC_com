											...
											<#if childrenMsiQuestionGroup.msiQuestionVos ??>
												<#list childrenMsiQuestionGroup.msiQuestionVos as msiQuestion>
													...
													<div class="msi_suv <#if questionType==1 || questionType==2>parent</#if>" questionId="${msiQuestion.msiQuestionId}">
														...
														<#if questionType =="3">
															<input type="text" <#if msiQuestion.isEditable==0>readonly="readonly"</#if> class="parameters <#if msiQuestion.msiQuestionId!=309 && msiQuestion.msiQuestionId!=308>required</#if> <#if msiQuestion.msiQuestionId==140>intType</#if>" id="${msiQuestion.msiQuestionId}_col1" name="${msiQuestion.msiQuestionId}_col1" value="${msiQuestion.col1}">
														</#if>
													</div>
													    ...
													    <div id="fileList_${objectId}" class="uploader-list">
													    	<#list msiQuestion.msiSurveyFeedbackAttachments as atts>
													    		...
													    		<div class="info_del"></div>
													    			<!--
													    				<a href="/audio/downLoad?fileName=${clientId}/web/${atts.attachment}">下载</a>
													    			 -->
	...
	$(document).ready(function () {
		...
		$("#savetButton").bind("click",function(){
			...
			if(dataFlag){
				...
				$("body").showLoading();
				$.ajax({
					url : "${contextPath}/appleMsiSurvey/updateMsiSurveyData",
					type : "post",
					dataType:"json",
					async: false,
