...
public class ClientUserController extends BaseActionSupport {
	...
	public String query(Model model, Integer page, String name,Integer clientStructureId,Integer clientPositionId,String loginName,Integer isActivation,Integer status, String structureName, Integer parentId,Integer rolesId, Integer isLower, String mod, HttpServletRequest req) throws BusinessException{
...
//			String queryColumn
	...
	public void exportClientUser(String name,Integer clientStructureId,Integer clientPositionId,String loginName,Integer isActivation,Integer status, String structureName,Integer parentId,Integer rolesId,HttpServletRequest req, HttpServletResponse resp) throws UnsupportedEncodingException,IllegalAccessError{
		...
		parameters.put("parentId", parentId);
		parameters.put("rolesId", rolesId);
		List<ClientUser> clientUserList = clientUserService.exportClientUserByParams(parameters);
	...
	public String showImportDialog(Model model){
		...
		}else if(8 == getClientId()){
			tempURL = "/download/import_template/8/user_import_template.xlsx";
		}else if(Constants.CLINT_ID_FERRERO.equals(getClientId())){
			tempURL = "/download/import_template/11/user_import_template.xlsx";
		}else {
	...
	@ResponseBody
	public ResultMessage importClientUser(MultipartFile file, HttpServletRequest request,HttpServletResponse resp) {
		Integer clientId = getClientId();
		if(clientId.equals(Constants.CLINT_ID_WORK)){
			ResultMessage resultMessage = clientUserService.addImportWorkLog(file, request, resp, clientId);
			return resultMessage;
		}else if(clientId.equals(Constants.CLINT_ID_FERRERO)){
			ResultMessage resultMessage = clientUserService.addImportFerreroLog(file, request, resp, clientId);
			return resultMessage;
		}
	...
	public String showEditClientUser(@PathVariable("clientUserId")Integer clientUserId, Model model, String mod) throws BusinessException{
		...
		if(mod !=null && mod.equals(Constants.DATA_MOD_CONF)) {
			...
			reFtl = "base/autoShowEditClientUser";
		}
	...
	public String showDtail(@PathVariable("clientUserId")Integer clientUserId, Model model,String mod)throws BusinessException{
		...
		clientUser.setRoleNames(roleNames);
		model.addAttribute("clientUser" , clientUser);
		model.addAttribute("clientRolesUserMappingList" , clientRolesUserMappingList);
		return reFtl;
		}
		return "/dialog/error";	
	}
...
//		List<Province> listprovince = provinceService.getAll();
		if(mod !=null && mod.equals(Constants.DATA_MOD_CONF)) {
			...
			reFtl = "base/autoShowAddClientUser";
		}
		
//		model.addAttribute("cpList", cpList);
//		model.addAttribute("listprovince", listprovince);
		model.addAttribute("clientId", clientId);
//		model.addAttribute("clientRolesList", clientRolesList);
		return reFtl;
	}
...
//		clientUserExpandService.updateUserLoginInfo(clientUserId, null, null, null, clientId);
	...
	@RequestMapping(value="getClientUserPositionBystoreId",produces="application/json")
	@ResponseBody
	public List<ClientUser> getClientUserPositionBystoreId(Integer clientUserId,String storeId,String q,Integer page){
		int clientId = getClientId();
		String name = q;
		Integer clientStructureId = getClientStructureId();
		/**获取组织架构级别该部门的所有子部门**/
		...
		String deptIds = StringUtil.stringIntersect(subAllStructureId, subStructureId);
		String subordinates = channelCommService.getSubordinatesWithOutSelf(getCurrentUserId().toString());
		List<ClientUser> list = clientUserService.getClientUserPositionBystoreId(clientUserId,clientId,name,deptIds,subordinates,storeId);
		return list;
	}	
...
//		ClientUser clientUser= clientUserService.selectByPrimaryKey (getCurrentUserId());
	...
	public String showReplaceClientUser(Integer clientStructureId,Model model){
		...
		model.addAttribute("clientStructureId", clientStructureId);
		model.addAttribute("clientId", clientId);
		return "base/showChildClientUser";
	}
	/**
	 * 替换人员嵌入页面
	 * @param model
	...
	public String replaceUpClientUser(Model model, Integer page, String name,Integer clientStructureId,Integer clientUserId,Integer status,Integer clientPositionId,HttpServletRequest req){
		int clientId = getClientId ();
		clientStructureId = clientStructureId !=null ? clientStructureId : getClientStructureId();
		/**获取组织架构级别该部门的所有子部门**/
		String subAllStructureId = channelCommService.getSubStructrueIds(clientStructureId);
		...
		return "base/showReplaceUpClientUser";
	}
	 ...
	 * @return
	 */
	@RequestMapping(value = "/showSynchroEHR")
	public String showSynchroEHR(){
		
		return "/base/showSynchroEHR";
	}
	 ...
	 * @return
	 */
	@RequestMapping(value = "/synchroEHR")
	@ResponseBody
	public ResultMessage synchroEHR(){
		...
		try {
			...
			String time = clientUserService.updateClientUser(super.getClientId(),clinetUsers,citys,execTimeLog,structuresMap);
			if(StringUtil.isEmptyString(time)){
				message = new ResultMessage(ResultMessage.ERROR, "同步失败");
				Map<String, Object> attributes = new HashMap<String, Object>();
				attributes.put("time", "同步失败");
				...
				message = new ResultMessage(ResultMessage.SUCCESS, "同步成功");
				Map<String, Object> attributes = new HashMap<String, Object>();
				attributes.put("time", "同步成功，最新同步时间："+DateTimeUtils.formatTime(new Date(),DateTimeUtils.yyyyMMddHHmmss));
				message.setAttributes(attributes);
			}
	 ...
	 * @return
	 */
	@RequestMapping(value = "/showimportClientUser")
	public String showimportClientUser(Model model){
		int clientId = getClientId();
		model.addAttribute("clientId", clientId);
		return "base/importClientUser";
	}
		...
		Map<String,Object> message = new HashMap<String, Object>();
			try {
				if(null==file){
					...
					//角色
				    Map<String, ClientRoles> mapClientRoles=clientRolesService.selectAllRoles(super.getClientId());
				    //职位
				    Map<String,ClientPosition> mapClientPosition=clientPositionService.selectAllClientPosition(super.getClientId());
				   
				    Map<String,ClientUser> mapUser =clientUserService.getClientUserByClientName(super.getClientId());//上级
				    
					message = clientUserService.importClientUser(values,mapUser, mapDept,mapCity, mapProvince,mapClientRoles,mapClientPosition, super.getClientId());
