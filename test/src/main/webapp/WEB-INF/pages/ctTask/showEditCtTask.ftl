...
<#if ctTaskDataSerchVo.clientId==7>
	<form action="" class="form-horizontal">	
		<div class="control-group">
			<div class="fl">
				<label class="control-label fl" for="clientUserId"><i style="color:red">*</i>关联人员：</label> 
				<div class="controls">
				  <input type="text" controlType="select" id="clientUserId" name="clientUserId" value="${ctTaskData.clientUserId}" class="input-medium require" <#if clientUserId??>readonly</#if>>
				</div>
			</div>
		</div> 
	</form>	
...
<form id="dataForm" action="" class="form-horizontal" method="post">
										    ...
										    <div id="fileList_${objectId}" class="uploader-list">
											    	<#if parameterMap[key]?exists>
												    	<#list parameterMap[key]?split(",") as imgName>
												    		<div id="E_WU_FILE_${imgName_index}" class="file-item thumbnail upload-state-done" val="${imgName}">
					...
					<#list ctTaskParameters as ctTaskParameter>
						<tr>
			      			...
			      			<td>
								...
								<#elseif  ctTaskParameter.controlType=='11'>
									<#assign objectId="${ctTaskParameter.ctTaskParameterId}_">
										<div id="uploader-demo" class="wu-example" style="width:800px;">
										    <input type="hidden" id="${objectId}" name="${objectId}" />
										    <div id="fileList_${objectId}" class="uploader-list">
										    	<#if parameterMap[key]?exists>
	...
	<div id="uploader-demo" class="wu-example attachment" style="width:730px;">
	    <input type="hidden" id="imageNames" name="imageNames" />
	    <div id="fileList" class="uploader-list">
	    	<#if ctTaskData??&&ctTaskData.ctTaskDataAttachments??>
...
<!--<div id="div_step_detail" class="div_step_detail" style="width: 100%;height:100%;border:none;background: #fff;margin-left:0px;">-->
	</div>
...
$(function(){	 
	...
	$("#savetButton").bind("click",function(){
		...
		var imageNames = getAttachmentUploaderVal();
		var dataFlag = "true";
		$(".require").each(function(){
 		...
 		$(".intType").each(function(){			
			...
			$.post("${contextPath}/ctTaskData/getCtTaskDataByStartTime",{ctTaskId:ctTaskId,popId:popId,visitDateStr:visitDate},function(result){
				ctTaskDataId = result['ctTaskDataId'];
			...
			if(dataFlag=="true"){
				var dataString = $("#dataForm").serialize();	 	
		 		...
		 		var ctTaskData = new Object();
			 		if('${ctTaskDataSerchVo.clientId}'=='7'){
			 			var clientUserId = $("#clientUserId").select2("val");
			 			ctTaskData.clientUserId = clientUserId;
			 		}
		 			ctTaskData.popId = storeId;
		 			ctTaskData.ctTaskId = ctTaskId;
		 			ctTaskData.startTime = startTime;
...
function getAttachmentUploaderVal(){
	var uploadobj = $(".attachment .upload-state-done");
	var value = '';
