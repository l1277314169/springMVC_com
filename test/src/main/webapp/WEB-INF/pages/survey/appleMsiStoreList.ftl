...
$(function(){
	...
	$(".addMsi").bind("click",function(){
		...
		$.post("${contextPath}/appleMsiSurvey/findMsiSurveyFeedbackByStoreId/"+storeId+"/",function(data){
			if(data!=null && data!=''){
				layer.alert("该门店已被访问");
			}else{
				var url = "${contextPath}/appleMsiSurvey/showAddAppleMsiSurveyData/"+storeId+"/"+storeNo+"/"+storeName;
				window.location.href=url;
			}	
		});
	...
	$("a.addHistoryMsi").bind("click",function(){	      
		var storeId = $(this).attr("dataId");
		var url = "${contextPath}/appleMsiSurvey/findHistoryMsiSurveyList/"+storeId;
		addHisDialog = new xDialog(url,{},{title:"历史问卷",width:400,height:300 }); 
		return false;	
	});
});
</script>