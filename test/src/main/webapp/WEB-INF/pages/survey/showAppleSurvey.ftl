			  		...
			  		$.each(data, function(index, v) {
			  			if(v.controlType==7){
			  				...
			  				for (var i = 0; i < vals.length; i++) {
			  					...
			  					var html = '<div id="E_WU_FILE_'+index+'" class="file-item thumbnail" val="'+vals[i]+'"><img href="/uploadfiles/'+clientId+'/web/thumbnail/xl_'+vals[i]+'" src="/uploadfiles/'+clientId+'/web/thumbnail/l_'+vals[i]+'" /><a target="_black" style="position:absolute;margin-left:28px" href="${contextPath}/uploadfiles/${clientId}/web/'+vals[i]+'">原图</a></div>';
