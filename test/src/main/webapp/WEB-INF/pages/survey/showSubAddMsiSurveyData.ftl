...
<div>
	<div class="widget-content nopadding">
		<form class="form-horizontal" action="#" id="dataForm" method="post">
			...
			<input type="hidden" id="msiSurveyId" name="msiSurveyId" value="${msiSurvey.msiSurveyId!''}"/>
			...
			<div class="control-group">
				<div nowrap="true" class="fl">
					...
					<label class="control-label">${store.storeNo}</label>
				</div>
				<div nowrap="true" class="fl">
					<label class="control-label">门店名称：</label>
					<label class="control-read"><#if storeName?length gt 15>${store.storeName?substring(0,15)}…<#else>${store.storeName}</#if></label>
				</div>
				<div nowrap="true" class="fl">
			...
			<div class="control-group">
				...
				<div nowrap="true" class="fl">
					...
					<div class="controls">
						<input type="text" id="promoter" name="promoter" class="input-medium required" <#if clientUser??>readonly="readonly" style="background-color:#F5F5F5"</#if> value="<#if clientUser??>${clientUser.name}</#if>"/>
					</div>
				</div>
				<div nowrap="true" class="fl">
					<label class="control-label fl" for="promoterNo">促销员工号：</label>
					<div class="controls">
						<input type="text" id="promoterNo" name="promoterNo" class="input-medium required" <#if clientUser??>readonly="readonly" style="background-color:#F5F5F5"</#if> value="<#if clientUser??>${clientUser.userNo}</#if>"/>
					</div>
				</div>
			</div>
			<div class="control-group">
	...
	$(document).ready(function () {
		...
		$("#savetButton").bind("click",function(){
			...
			if(dataFlag){
				...
				$("body").showLoading();
				$.ajax({
					url : "${contextPath}/msiSurveyFeedback/saveMsiSurveyData",
					type : "post",
					dataType:"json",
					async: false,
					...
					success : function(result) {
						$("body").hideLoading();
						var feedbackId = result['feedbackId'];
						$("#feedbackId").attr("value",feedbackId);
						$(".tab_current").attr("feedbackId",feedbackId);
						$(".tab_current").attr("promoterNo",$("#promoterNo").val());
						$(".tab_current b").html($("#promoter").val());
						$("#savetButton").removeAttr("disabled");
						layer.alert("保存成功");
				   	},
