...
<input type="hidden" id="planningId" name="planningId" value="${callPlanning.planningId}">
	  <table class="table_white_bg">
            <tbody>
            	<tr>
                	<td class="td_label" style="line-height:50px"><i class="cc1">*</i>人员：</td>
                    <td class="controls"><input type="text" id="clientUserId" name="clientUserId"  value="${callPlanning.clientUserId}" required readOnly="true"></td>
                </tr> 
                <tr>
                	<td  class="td_label" style="line-height:50px"><i class="cc1">*</i>任务内容：</td>
                    <td class="controls">
	                   <input type="text" id="enumValue" name="enumValue" value="${callPlanning.enumValue}" required>
                    </td>
                </tr> 
                <tr>
                	<td  class="td_label" style="line-height:45px">计划日期：</td>
                	 <td class="controls">
	                   <input type="text" id="callDate" name="callDate" value="<#if callPlanning.callDate??>${callPlanning.callDate?string("yyyy-MM-dd")}</#if>" class="input-medium" readonly="true">
                    </td>
                </tr>
                 <tr>
        ...
        <input type="hidden" id="oldCallDate" name="oldCallDate" value="<#if callPlanning.callDate??>${callPlanning.callDate?string("yyyy-MM-dd")}</#if>">
</form>
<script>
...
$('#oldSavetButton').on("click",function(){
	...
	$.ajax({
		url : "${contextPath}/callPlanning/editNotCallPlanning",
		type : "post",
		dataType:"json",
		async: false,
