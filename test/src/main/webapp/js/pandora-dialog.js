(function (global, $, undefined) {
    ...
    Dialog.prototype = {
        ...
        _mask: function (config) {
            ...
            this._mask = function () {
                $overlay.show();
                $("[data-mask=overlay]").css("display", "block");
            }
            
            // 有待完善 HACK
            if ($overlay.size() === 0) {
                var div = document.createElement('div');
                $(div).attr("data-mask", "overlay").addClass(config.maskClass).css("zIndex", config.zIndex - 1);
        ...
        loading: function (content) {
            var html = '<div style="text-align:center">' +
                       '<img width="46" height="46" src="/blank.gif"' +
                       'alt="loading..." ></div>';
    ...
    $.confirm = pandora.confirm = function (content, ok, cancel) {
        return Factory({
            ...
            lock: true,
            mask: true,
            content: "<div class='dialog-confirm'>"+content+"</div>",
            ok: ok ? ok : true,
    ...
    $.loading = pandora.loading = function (content, callback) {
        return Factory({
            ...
            content: '<p><img src="/loading.gif" alt="loading" /></p>' + content,
            initialize: callback
        });
    };
