...
function drawHBarChart(boxID, data) {
    ...
    for (var i = 0; i < data.length; i++) {
        ...
        jQuery('<span/>', {
            id: 'promoterLabel'+boxID+ i,
            css: {
                'font-size': '12px',
                'font-family': 'Helvetica, Arial, sans-serif',
                'color': '#666666'
            }
        }).appendTo('#promoter'+boxID+ i);

        $('#promoterLabel'+boxID + i).html(data[i].Label);

        jQuery('<div/>', {
            id: 'promoterChart' +boxID+ i,
            css: {
                'width': '80%',
        ...
        jQuery('<div id="promoterGraph'+boxID+ i + '" style="float:left;width:0px;" class="hbarchart"/>', {
            id: 'promoterGraph'+boxID + i,
            css: {

        }
    }).appendTo('#promoterChart'+boxID + i);



    jQuery('<span/>', {
        id: 'promoterGraphVal'+boxID + i,
        css: {
            'float': 'left',
            'font-size': '12px',
            'font-weight': 'bold',
    ...
    var _html = data[i].Value;
    if(i==0){
    	_html+="%";
    }
    $('#promoterGraphVal'+boxID + i).html(_html);
    $('#promoterGraphVal'+boxID + i).hide();
    var lab = $('#promoterGraphVal'+boxID + i);
...
if (data[i].Value > 0) {
    ...
    var idx = boxID+i;
    $('div#promoterGraph'+boxID + i).animate({ width: widthVal }, 1000, '', showPromoterGraphValLabel(idx));
}
else {
    $('#promoterGraphVal'+boxID + i).show();
}
